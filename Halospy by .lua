...
local Highlight = (isfile and loadfile and isfile("Highlight.lua") and loadfile("Highlight.lua")()) or loadstring(game:HttpGet("https://raw.githubusercontent.com/ibrahimkhacef5-del/Simple-spy-2026-/refs/heads/main/simplespy.lua"))()
...
-- ============================================================
-- إعدادات الألوان (واجهة بسيطة وأنيقة)
-- ============================================================
local Colors = {
    Background = Color3.fromRGB(25, 27, 35),
    TopBar = Color3.fromRGB(35, 37, 45),
    LeftPanel = Color3.fromRGB(40, 42, 52),
    RightPanel = Color3.fromRGB(30, 32, 42),
    CodeBox = Color3.fromRGB(20, 22, 30),
...
-- ============================================================
-- إنشاء الواجهة الرئيسية (أصغر حجماً)
-- ============================================================
local SimpleSpy3 = Create("ScreenGui", {ResetOnSpawn = false})

-- الخلفية الرئيسية (مصغرة)
...
-- ============================================================
-- اللوحة اليسرى (قائمة الـ Remotes)
-- ============================================================
local LeftPanel = Create("Frame", {
    Parent = Background,
    BackgroundColor3 = Colors.LeftPanel,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 0, 0, 26),
    Size = UDim2.new(0, 130, 0, 254)
})
...
-- ============================================================
-- اللوحة اليمنى (صندوق الكود + الأزرار)
-- ============================================================
local RightPanel = Create("Frame", {
    Parent = Background,
    BackgroundColor3 = Colors.RightPanel,
    BorderSizePixel = 0,
    Position = UDim2.new(0, 130, 0, 26),
    Size = UDim2.new(0, 290, 0, 254)
})
...
local CodeBox = Create("Frame", {
    Parent = RightPanel,
    BackgroundColor3 = Colors.CodeBox,
    BorderSizePixel = 0,
    Size = UDim2.new(0, 290, 0, 135)
})
...
-- ============================================================
-- تأثيرات Hover (حركة سلسة)
-- ============================================================
CloseButton.MouseEnter:Connect(function()
    TweenService:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(255, 60, 60)}):Play()
end)
CloseButton.MouseLeave:Connect(function()
    TweenService:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(50, 45, 55)}):Play()
end)

MaximizeButton.MouseEnter:Connect(function()
    TweenService:Create(MaximizeButton, TweenInfo.new(0.2), {BackgroundColor3 = Colors.Accent3}):Play()
end)
MaximizeButton.MouseLeave:Connect(function()
    TweenService:Create(MaximizeButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(50, 55, 70)}):Play()
end)

MinimizeButton.MouseEnter:Connect(function()
    TweenService:Create(MinimizeButton, TweenInfo.new(0.2), {BackgroundColor3 = Colors.Accent2}):Play()
end)
MinimizeButton.MouseLeave:Connect(function()
    TweenService:Create(MinimizeButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(50, 55, 70)}):Play()
end)

-- ============================================================
-- تأثير تحريك أكبر للواجهة عند السحب
-- ============================================================
local dragData = {isDragging = false, dragStart = nil, startPos = nil}

TopBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        dragData.isDragging = true
        dragData.dragStart = UserInputService:GetMouseLocation()
        dragData.startPos = Background.Position
    end
end)
...
RunService.RenderStepped:Connect(function()
    if dragData.isDragging then
        ...
        local offset = mousePos - dragData.dragStart
        local newX = dragData.startPos.X.Offset + offset.X
        local newY = dragData.startPos.Y.Offset + offset.Y
        
        -- منع الخروج من الشاشة
        local viewport = workspace.CurrentCamera.ViewportSize
        newX = math.max(0, math.min(newX, viewport.X - Background.AbsoluteSize.X))
        newY = math.max(0, math.min(newY, viewport.Y - Background.AbsoluteSize.Y - 36))
...
-- ============================================================
-- باقي الكود الأصلي للـ SimpleSpy (الدوال والأحداث)
-- ============================================================
-- ...

-- باقي الكود الأصلي للـ SimpleSpy (الدوال والأحداث) يبقى كما هو...
...
-- ============================================================
-- الأزرار الرئيسية (نسخة عربية)
-- ============================================================

-- نسخ الكود
newButton(
    "نسخ الكود",
    function() return "انسخ الكود المعروض في صندوق العرض" end,
    function()
        setclipboard(codebox:getString())
        TextLabel.Text = "✅ تم نسخ الكود!"
    end
)
...
newButton(
    "نسخ الريمـوت",
    function() return "انسخ مسار الريمـوت المحدد" end,
    function()
        if selected and selected.Remote then
            setclipboard(v2s(selected.Remote))
            TextLabel.Text = "✅ تم نسخ الريمـوت!"
        end
    end
)
...
newButton(
    "تشغيل الكود",
    function() return "نفذ الكود المعروض" end,
    function()
        local Remote = selected and selected.Remote
        if Remote then
            TextLabel.Text = "⏳ جاري التنفيذ..."
            xpcall(function()
                local returnvalue
                if Remote:IsA("RemoteEvent") or Remote:IsA("UnreliableRemoteEvent") then
                ...
                TextLabel.Text = "✅ تم التنفيذ بنجاح!"
            end, function(err)
                TextLabel.Text = "❌ خطأ: " .. err
            end)
            return
        end
        TextLabel.Text = "⚠️ لم يتم العثور على ريمـوت"
    end
)
    ...
    function()
        ...
        if func then
            ...
            if typeoffunc ~= 'string' then
                ...
                info = {
                    info = getinfo(func),
                    constants = lclosure and deepclone(getconstants(func)) or "N/A -- دالة C",
                ...
                codebox:setRaw("--[[ جاري التحويل... ]]")
                selected.Function = v2v({functionInfo = info})
            end
            codebox:setRaw("-- معلومات الدالة\n-- بواسطة Simple Spy V16\n\n" .. selected.Function)
            TextLabel.Text = "✅ تم إنشاء معلومات الدالة!"
        else
            TextLabel.Text = "❌ لم يتم العثور على الدالة"
        end
    end
)
...
newButton(
    "مسح السجلات",
    function() return "امسح جميع سجلات الريمـوت" end,
    function()
        TextLabel.Text = "⏳ جاري المسح..."
        clear(logs)
        ...
        codebox:setRaw("")
        selected = nil
        TextLabel.Text = "✅ تم مسح السجلات!"
    end
)

-- ============================================================
-- أزرار التحكم بالريمـوت
-- ============================================================

-- تجميد (فردي)
newButton(
    "تجميد (i)",
    function() return "تجميد هذا الريمـوت فقط (سيتم تجاهله)" end,
    function()
        if selected then
            blacklist[OldDebugId(selected.Remote)] = true
            TextLabel.Text = "❄️ تم تجميد الريمـوت!"
        end
    end
)
...
newButton(
    "استبعاد (n)",
    function() return "استبعاد جميع الريمـوتات بهذا الاسم" end,
    function()
        if selected then
            blacklist[selected.Name] = true
            TextLabel.Text = "🚫 تم الاستبعاد!"
        end
    end
)
...
newButton(
    "مسح القائمة السوداء",
    function() return "امسح قائمة الريمـوتات المستبعدة" end,
    function()
        blacklist = {}
        TextLabel.Text = "✅ تم مسح القائمة السوداء!"
    end
)
...
newButton(
    "حظر (i)",
    function() return "منع هذا الريمـوت من الإرسال للخادم" end,
    function()
        if selected then
            blocklist[OldDebugId(selected.Remote)] = true
            TextLabel.Text = "⛔ تم حظر الريمـوت!"
        end
    end
)
...
newButton(
    "حظر (n)",
    function() return "منع جميع الريمـوتات بهذا الاسم" end,
    function()
        if selected then
            blocklist[selected.Name] = true
            TextLabel.Text = "⛔ تم الحظر!"
        end
    end
)
...
newButton(
    "مسح الحظر",
    function() return "امسح قائمة الريمـوتات المحظورة" end,
    function()
        blocklist = {}
        TextLabel.Text = "✅ تم مسح الحظر!"
    end
)
    ...
    function()
        if decompile then
            if selected and selected.Source then
                ...
                codebox:setRaw(DecompiledScripts[Source] or "-- لم يتم العثور على المصدر")
                TextLabel.Text = "✅ تم فك التجميع!"
            else
                TextLabel.Text = "⚠️ لم يتم العثور على المصدر"
            end
        else
            TextLabel.Text = "❌ وظيفة فك التجميع غير متوفرة"
        end
    end
)
...
newButton(
    "تعطيل المعلومات",
    function() return string.format("[%s] تعطيل معلومات الدالة (لتقليل اللاج)", configs.funcEnabled and "✅ مفعل" or "❌ معطل") end,
    function()
        configs.funcEnabled = not configs.funcEnabled
        TextLabel.Text = string.format("معلومات الدالة: %s", configs.funcEnabled and "✅ مفعلة" or "❌ معطلة")
    end
)
...
newButton(
    "الحظر التلقائي",
    function() return string.format("[%s] كشف ومنع الريمـوتات المزعجة تلقائياً", configs.autoblock and "✅ مفعل" or "❌ معطل") end,
    function()
        configs.autoblock = not configs.autoblock
        TextLabel.Text = string.format("الحظر التلقائي: %s", configs.autoblock and "✅ مفعل" or "❌ معطل")
        history = {}
        excluding = {}
    end
)
...
newButton(
    "معلومات متقدمة",
    function() return string.format("[%s] عرض معلومات إضافية عن الريمـوت", configs.advancedinfo and "✅ مفعل" or "❌ معطل") end,
    function()
        configs.advancedinfo = not configs.advancedinfo
        TextLabel.Text = string.format("المعلومات المتقدمة: %s", configs.advancedinfo and "✅ مفعلة" or "❌ معطلة")
    end
)
...
newButton(
    "انستغرام",
    function() return "اذهب إلى حساب إنستغرام المطور" end,
    function()
        setclipboard("https://www.instagram.com/uzcawe")
        TextLabel.Text = "✅ تم نسخ رابط إنستغرام!"
        if request then
            pcall(function()
                request({
                    Url = 'https://www.instagram.com/uzcawe',
                    Method = 'GET'
                })
            end)
        end
...
newButton(
    "ديسكورد",
    function() return "انضم إلى سيرفر ديسكورد" end,
    function()
        setclipboard("https://discord.gg/CfDaAKZr")
        TextLabel.Text = "✅ تم نسخ رابط ديسكورد!"
        if request then
            pcall(function()
                request({
...
newButton(
    "📂 المصدر",
    function() return "عرض جميع ملفات المشروع على GitHub" end,
    function()
        setclipboard("https://github.com/ibrahimkhacef5-del/Simple-spy-2026-")
        TextLabel.Text = "✅ تم نسخ رابط المصدر!"
        if request then
            pcall(function()
                request({
                    Url = 'https://github.com/ibrahimkhacef5-del/Simple-spy-2026-',
                    Method = 'GET'
                })
            end)
        end
...
-- ============================================================
-- أزرار سرية (للمطورين فقط)
-- ============================================================

if configs.supersecretdevtoggle then
    newButton(
        "🔐 SSV2",
        function() return "تحميل Simple Spy V2.2" end,
        function()
    ...
    newButton(
        "🔐 SSV3",
        function() return "تحميل Simple Spy V3" end,
        function()
            loadstring(game:HttpGet("https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpySource.lua"))()
        end
    )
    
    -- زر سري لتشغيل موسيقى
    local SuperSecretFolder = Create("Folder", {Parent = SimpleSpy3})
    newButton(
        "🎵 سري",
        function() return "زر سري جداً 👀" end,
        function()
            ...
            if #random > 0 then
                ...
                NotSound:Play()
                TextLabel.Text = "🎵 تشغيل موسيقى!"
            else
                TextLabel.Text = "⚠️ لا توجد ملفات موسيقى"
            end
        end
    )
end
...
if table.find({
    ...
    -- ============================================================
    -- جعل الخلفية قابلة للسحب
    -- ============================================================
    Background.Draggable = true
    
    -- ============================================================
    -- إنشاء الزر الرئيسي (بسيط وبدون أخطاء)
    -- ============================================================
    ...
    QuickCapture.Name = "QuickCapture"
    
    -- خلفية الزر
    QuickCapture.BackgroundColor3 = Color3.fromRGB(30, 30, 60)
    QuickCapture.BackgroundTransparency = 0.1
    
    -- الموقع والحجم
    QuickCapture.Position = UDim2.new(0.85, 0, 0.92, 0)
    QuickCapture.Size = UDim2.new(0, 55, 0, 55)
    QuickCapture.Font = Enum.Font.SourceSansBold
    QuickCapture.Text = "●"
    QuickCapture.TextColor3 = Color3.fromRGB(255, 255, 255)
    QuickCapture.TextSize = 24
    ...
    UIStroke.Parent = QuickCapture
    
    -- ============================================================
    -- إنشاء النص "Simple Spy V16" (يظهر عند الضغط)
    -- ============================================================
    local TitleLabel = Instance.new("TextLabel")
    TitleLabel.Parent = QuickCapture
    TitleLabel.BackgroundColor3 = Color3.new(1, 1, 1)
    TitleLabel.BackgroundTransparency = 1
    TitleLabel.Size = UDim2.new(0, 130, 0, 20)
    TitleLabel.Position = UDim2.new(0.5, -65, 1.3, 0)
    TitleLabel.Font = Enum.Font.GothamBold
    TitleLabel.Text = "By uzcawe"
    ...
    -- ============================================================
    -- القائمة المنبثقة (بدون UIBlur لتجنب الأخطاء)
    -- ============================================================
    local MenuFrame = Instance.new("Frame")
    local MenuCorner = Instance.new("UICorner")
    local MenuStroke = Instance.new("UIStroke")
    
    MenuFrame.Parent = SimpleSpy3
    MenuFrame.Name = "MenuFrame"
    MenuFrame.BackgroundColor3 = Color3.fromRGB(20, 22, 35)
    MenuFrame.BackgroundTransparency = 0.05
    MenuFrame.Size = UDim2.new(0, 180, 0, 0)
    MenuFrame.Position = UDim2.new(0.5, -90, 0.4, 0)
    MenuFrame.ZIndex = 20
    ...
    -- ============================================================
    -- عناصر القائمة (بدون أخطاء)
    -- ============================================================
    
    -- عنوان القائمة
    local MenuTitle = Instance.new("TextLabel")
    MenuTitle.Parent = MenuFrame
    MenuTitle.BackgroundColor3 = Color3.new(1, 1, 1)
    MenuTitle.BackgroundTransparency = 1
    MenuTitle.Size = UDim2.new(1, 0, 0, 35)
    MenuTitle.Position = UDim2.new(0, 0, 0, 5)
    MenuTitle.Font = Enum.Font.GothamBold
    MenuTitle.Text = "✦ Simple Spy ✦"
    MenuTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
    MenuTitle.TextSize = 16
    MenuTitle.TextStrokeColor3 = Color3.fromRGB(0, 200, 255)
    MenuTitle.TextStrokeTransparency = 0.3
    
    -- أزرار القائمة
    local menuButtons = {}
    local buttonData = {
        {text = "📱 Open", color = Color3.fromRGB(100, 150, 255)},
        {text = "🔍 Look", color = Color3.fromRGB(255, 100, 150)},
        {text = "❌ Close", color = Color3.fromRGB(255, 80, 80)},
    }
    ...
    for i, data in ipairs(buttonData) do
        ...
        btn.Position = UDim2.new(0.075, 0, 0, 45 + (i - 1) * 45)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            elseif i == 2 then
                ...
                TweenService:Create(TitleLabel, TweenInfo.new(0.5), {
                    Position = UDim2.new(0.5, -65, 1.8, 0),
                    TextStrokeTransparency = 0
                }):Play()
                wait(0.8)
                TweenService:Create(TitleLabel, TweenInfo.new(0.5), {
                    Position = UDim2.new(0.5, -65, 1.3, 0),
            ...
            elseif i == 3 then
                ...
                QuickCapture.TextColor3 = Color3.fromRGB(255, 200, 0)
                MenuFrame.Visible = false
                MenuFrame.Size = UDim2.new(0, 180, 0, 0)
            end
        end)
    ...
    local lastTime = 0
    local targetHz = 90
    local frameTime = 1 / targetHz
    
    RunService.RenderStepped:Connect(function(deltaTime)
        lastTime = lastTime + deltaTime
        if lastTime >= frameTime then
            lastTime = 0
            
            -- تحديث موضع الزر مع حدود الشاشة
            local viewport = workspace.CurrentCamera.ViewportSize
            local posX = QuickCapture.Position.X.Offset
            local posY = QuickCapture.Position.Y.Offset
            
            -- منع الخروج من الشاشة
            if posX < 0 then posX = 0 end
            if posX > viewport.X - 60 then
                posX = viewport.X - 60
            end
            if posY < 0 then posY = 0 end
            if posY > viewport.Y - 60 - 36 then
                posY = viewport.Y - 60 - 36
            end
            
            QuickCapture.Position = UDim2.new(0, posX, 0, posY)
        end
    end)
    ...
    -- أنيميشن دخول الزر
    -- ============================================================
    QuickCapture.Size = UDim2.new(0, 0, 0, 0)
    QuickCapture.Position = UDim2.new(0.85, 0, 0.92, 50)
    
    TweenService:Create(QuickCapture, TweenInfo.new(0.5, Enum.EasingStyle.Back), {
    ...
    -- عند النقر على الخلفية
    Background.MouseButton1Click:Connect(closeMenu)
end